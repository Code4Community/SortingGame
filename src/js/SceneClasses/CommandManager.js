export default class CommandManager {
  ...
  defineIncrementalCommand(commandName) {
    C4C.Interpreter.define(commandName, () => {
      ...
      console.log(
        `[CommandManager] Incremental command '${commandName}' scheduled`,
      );
    });
  }
  ...
  defineQueuedCustomCommand(commandName, callback) {
    C4C.Interpreter.define(commandName, () => {
      ...
      console.log(
        `[${this.scene.currentLevel}] ${commandName} queued custom command executed.`,
      );
    });
  }
  ...
  executeCommand(commandName) {
    ...
    if (!pathName) {
      console.warn(
        `[${this.scene.currentLevel}] No path mapping found for command: ${commandName}`,
      );
      return;
    }
    ...
    if (!this.pathManager.paths[pathName]) {
      console.warn(
        `[${this.scene.currentLevel}] Path '${pathName}' not found for command '${commandName}'`,
      );
      return;
    }
